...
bool load_image(std::vector<std::vector<unsigned u32>>& image, const std::string& filename, int& x, int&y)
{
    int n;
    ...
    if (data != nullptr)
    {
        ...
        int index=0;
        image=std::vector<std::vector<unsigned u32>>(y);
        for(int i=0; i<y; i++)
        {
            image[i]=std::vector<unsigned u32>(x);
            for(int j=0; j<x; j++)
            {
...
bool load_image_invert(std::vector<std::vector<unsigned u32>>& image, const std::string& filename, int& x, int&y)
{
    int n;
    unsigned char* data = stbi_load(filename.c_str(), &x, &y, &n, 4);
    if (data != nullptr)
...
struct texture
{
std::vector<std::vector<unsigned u32>>image;
...
recpos=0; materialpos=0;
recpos2=0; materialpos2=0;
texture="";
}
